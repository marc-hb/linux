			   ...
			   unsigned long exit_qualification)
{
	...
	if (!(vmcs12->vm_exit_reason & VMX_EXIT_REASONS_FAILED_VMENTRY)) {
		...
		/*
		 * In FRED event delivery, the value of the nested exception bit of
		 * VM-exit interruption information (aka exiting-event identification)
		 * is always identical to that of the valid bit of the IDT-vectoring
		 * information (aka original-event identification).
		 */
		if ((vmcs12->idt_vectoring_info_field & VECTORING_INFO_VALID_MASK) &&
		    (vmcs12->vm_entry_controls & VM_ENTRY_IA32E_MODE) &&
		    (vmcs12->guest_cr4 & X86_CR4_FRED))
