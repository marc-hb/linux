...
static int imh_get_imc_num(struct res_config *cfg)
{
	...
	if (imc_num > IMH_NUM_IMC) {
		imh_printk(KERN_ERR, "Please make IMH_NUM_IMC >= %d\n", imc_num);
		return -EINVAL;
	}

	edac_dbg(2, "Get ddr mc number %d\n", imc_num);
...
/* TODO: Remove the DMR CPUID when posting the patch to community. */
/* Family 19 */
#define INTEL_PANTHERCOVE_X            IFM(19, 0x01) /* Diamond Rapids */
static const struct x86_cpu_id imh_cpuids[] = {
	X86_MATCH_VFM(INTEL_PANTHERCOVE_X, &dmr_cfg),
	{}
};
