...
static int hpwdt_init_nmi_decoding(struct pci_dev *dev)
{
	 ...
	 */
	retval = register_nmi_handler(NMI_UNKNOWN, hpwdt_pretimeout, NMI_FLAG_NONE, "hpwdt");
	if (retval)
		goto error;
	retval = register_nmi_handler(NMI_SERR, hpwdt_pretimeout, NMI_FLAG_NONE, "hpwdt");
	if (retval)
		goto error1;
	retval = register_nmi_handler(NMI_IO_CHECK, hpwdt_pretimeout, NMI_FLAG_NONE, "hpwdt");
	if (retval)
		goto error2;
