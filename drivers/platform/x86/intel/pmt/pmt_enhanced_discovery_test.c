...
static void validate_pmt_regions(struct kunit *test, struct pmt_feature_group *feature_group, int feature_id)
{
	int i;
	...
	for (i = 0; i < feature_group->count; i++) {
		...
		kunit_info(test, "\t\taddr=%p, size=%lu, num_rmids=%u", region->addr, region->size, region->num_rmids);
		...
		KUNIT_ASSERT_NOT_ERR_OR_NULL(test, region->addr);
	}
}
...
static struct kunit_case intel_pmt_enhanced_discovery_test_cases[] = {
	KUNIT_CASE(test_intel_pmt_get_regions_by_feature),
	{}
};

static struct kunit_suite intel_pmt_enhanced_discovery_test_suite = {
	.name = "pmt_enhanced_discovery_test",
	.test_cases = intel_pmt_enhanced_discovery_test_cases,
};

kunit_test_suite(intel_pmt_enhanced_discovery_test_suite);

MODULE_AUTHOR("David E. Box <david.e.box@linux.intel.com>");
MODULE_DESCRIPTION("Intel PMT Enhanced Dsicovery KUNIT test driver");
MODULE_LICENSE("GPL");
