...
#define GUID_SIZE(x)		((x) * sizeof(u32))
...
DEFINE_MUTEX(feature_list_lock);
			 ...
			 char *buf)
{
	...
	while (*pmt_caps) {
		...
		while (pmt_cap->name) {
			rc += sysfs_emit_at(buf, rc, "%-40s Available: %s\n",
					pmt_cap->name,
					pmt_cap->mask & caps ? "Yes" : "No");
			++pmt_cap;
		}
		++pmt_caps;
	}
...
static struct kobj_attribute num_rmids_attribute =
	__ATTR_RO(num_rmids);

static ssize_t min_watcher_period_show(struct kobject *kobj,
				       struct kobj_attribute *attr, char *buf)
{
	struct feature *feature = to_pmt_feature(kobj);
	...
	return sysfs_emit(buf, "%d ms\n", watcher->period);
}
static struct kobj_attribute min_watcher_period_attribute =
	__ATTR_RO(min_watcher_period);
...
static struct attribute *pmt_feature_rmid_attrs[] = {
	...
	&min_watcher_period_attribute.attr,
			   ...
			   struct feature_discovery_table *disc_tbl)
{
	...
	if (priv->mask & BIT(disc_tbl->id)) {
		dev_err(priv->dev, "Duplicate feature: %s\n",
			      ...
			      struct resource *disc_res)
{
	...
	case ACCESS_LOCAL:
		if (tbir) {
			dev_warn(priv->dev, "Unsupported BAR index %u for access type %u\n",
				tbir, disc_tbl->access_type);

			/* XXX: Emulation W/A */
			//return -EINVAL;
			disc_tbl->offset &= ~DT_TBIR;
		}
		 ...
		 */
		res.start = disc_res->end + disc_tbl->offset + 1;
		break;
	...
	res.end = res.start + disc_tbl->size * sizeof(u32) - 1;
	...
	/* Validate fields are as large as mapped resource */
	if (1) {
		size_t size = sizeof(*header) +
			      FEAT_ATTR_SIZE(header->attr_size) +
			      GUID_SIZE(header->num_guids);

		if (WARN(size > resource_size(&res), "%ld > %lld", size, resource_size(&res)))
			return -EINVAL;
	}

	/* The feature attributes, including caps */
	...
	/* The guids */
	guids = devm_kmalloc(priv->dev, GUID_SIZE(header->num_guids),
			     GFP_KERNEL);
	if (!guids)
		return -ENOMEM;
	...
	memcpy_fromio(guids, feat_offset, GUID_SIZE(header->num_guids));
...
/* Get the discovery table and uses it to get the feature table */
...
{
	struct feature *cur, *tmp;

	mutex_lock(&feature_list_lock);
	list_for_each_entry_safe(cur, tmp, &pmt_feature_list, list) {
		if (cur == feature) {
			list_del(&cur->list);
			mutex_unlock(&feature_list_lock);
			return;
...
static int pmt_features_probe(struct auxiliary_device *auxdev, const struct auxiliary_device_id *id)
{
	...
	if (IS_ERR(priv->dev)) {
		dev_err(priv->dev, "Could not create %s-%s device node\n",
...
static int __init pmt_features_init(void)
{
	return auxiliary_driver_register(&pmt_features_aux_driver);
}
module_init(pmt_features_init);

static void __exit pmt_features_exit(void)
{
	auxiliary_driver_unregister(&pmt_features_aux_driver);
}
module_exit(pmt_features_exit);
