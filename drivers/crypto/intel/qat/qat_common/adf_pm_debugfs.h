...
#ifndef ADF_PM_DEBUGFS_H_
#define ADF_PM_DEBUGFS_H_
...
#define PM_INFO_REGSET_ENTRY_MASK(_reg_, _field_, _mask_)          \
	{                                                          \
		...
		.key = __stringify(_field_), .field_mask = _mask_, \
	}
...
int pm_scnprint_table_upper_keys(char *buff, const struct pm_status_row *table,
				 u32 *pm_info_regs, size_t buff_size,
				 int table_len);

int pm_scnprint_table_lower_keys(char *buff, const struct pm_status_row *table,
				 u32 *pm_info_regs, size_t buff_size,
				 int table_len);

#endif /* ADF_PM_DEBUGFS_H_ */
