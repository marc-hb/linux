				 ...
				 void __iomem *csr, u32 iastatssm)
{
	...
	reg &= ADF_GEN6_UERRSSMSH_BITMASK;
	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal error on ssm shared memory: 0x%x\n", reg);
				 ...
				 void __iomem *csr, u32 iastatssm)
{
	...
	reg &= ADF_GEN6_PPERR_BITMASK;
	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal push or pull data error: 0x%x\n", reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
		ADF_CSR_WR(csr, ADF_GEN6_PPERR, reg);
	}
}

static void adf_handle_scmpar_err(struct adf_accel_dev *accel_dev,
				  void __iomem *csr, u32 iastatssm)
{
	u32 reg;
	...
	reg &= ADF_GEN6_SCM_PAR_ERR_BITMASK;
	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal error on SCM: 0x%x\n", reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
		ADF_CSR_WR(csr, ADF_GEN6_SSM_FERR_STATUS, reg);
	}
}

static void adf_handle_cpppar_err(struct adf_accel_dev *accel_dev,
				  void __iomem *csr, u32 iastatssm)
{
	u32 reg;
	...
	reg = ADF_CSR_RD(csr, ADF_GEN6_SSM_FERR_STATUS);
	reg &= ADF_GEN6_CPP_PAR_ERR_BITMASK;
	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal error on CPP: 0x%x\n", reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
		ADF_CSR_WR(csr, ADF_GEN6_SSM_FERR_STATUS, reg);
	}
}

static void adf_handle_rfpar_err(struct adf_accel_dev *accel_dev,
				 void __iomem *csr, u32 iastatssm)
{
	u32 reg;
	...
	reg = ADF_CSR_RD(csr, ADF_GEN6_SSM_FERR_STATUS);
	reg &= ADF_GEN6_RF_PAR_ERR_BITMASK;
	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal error on RF Parity: 0x%x\n", reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
		ADF_CSR_WR(csr, ADF_GEN6_SSM_FERR_STATUS, reg);
	}
}

static void adf_handle_unexp_cpl_err(struct adf_accel_dev *accel_dev,
				     void __iomem *csr, u32 iastatssm)
{
	u32 reg;
	...
	reg = ADF_CSR_RD(csr, ADF_GEN6_SSM_FERR_STATUS);
	reg &= ADF_GEN6_UNEXP_CPL_ERR_BITMASK;

	if (reg) {
		dev_err(&GET_DEV(accel_dev),
			"Fatal error for AXI unexpected tag/length: 0x%x\n",
			reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
		ADF_CSR_WR(csr, ADF_GEN6_SSM_FERR_STATUS, reg);
	}
				    ...
				    void __iomem *csr)
{
	...
	iastatssm &= ADF_GEN6_IAINTSTATSSM_BITMASK;
	if (!iastatssm)
		return;
				   ...
				   void __iomem *csr, u32 errsou)
{
	...
	if (reg & (ADF_GEN6_CPP_CFC_ERR_STATUS_ERR_BIT |
		   ADF_GEN6_CPP_CFC_ERR_STATUS_MERR_BIT)) {
		dev_err(&GET_DEV(accel_dev),
			"CPP_CFC_ERR: errors: 0x%x", reg);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
	}

	ADF_CSR_WR(csr, ADF_GEN6_CPP_CFC_ERR_STATUS_CLR,
		   ADF_GEN6_CPP_CFC_ERR_STATUS_CLR_BITMASK);
}

static void adf_handle_ssm(struct adf_accel_dev *accel_dev, void __iomem *csr,
			   u32 errsou)
{
...
static void adf_gen6_process_errsou2(struct adf_accel_dev *accel_dev,
				     void __iomem *csr, u32 errsou)
{
...
static void adf_gen6_process_errsou0(struct adf_accel_dev *accel_dev,
				     void __iomem *csr)
{
	u32 ae_corr_err = ADF_CSR_RD(csr, ADF_GEN6_HIAECORERRLOG_CPP0);

	ae_corr_err &= GET_HW_DATA(accel_dev)->ae_mask;

	dev_warn(&GET_DEV(accel_dev),
		 "Correctable error detected: 0x%x\n",
		 ae_corr_err);

	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_CORR);

	/* Clear interrupt from ERRSOU0 */
	ADF_CSR_WR(csr, ADF_GEN6_HIAECORERRLOG_CPP0, ae_corr_err);
}

static void adf_handle_cpp_ae_unc(struct adf_accel_dev *accel_dev,
				  void __iomem *csr, u32 errsou)
{
	u32 ae_uncorr_err;
	...
	ae_uncorr_err = ADF_CSR_RD(csr, ADF_GEN6_HIAEUNCERRLOG_CPP0);
	ae_uncorr_err &= GET_HW_DATA(accel_dev)->ae_mask;
	if (ae_uncorr_err) {
		dev_err(&GET_DEV(accel_dev),
			"Uncorrectable error detected in AE: 0x%x\n",
			ae_uncorr_err);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
		ADF_CSR_WR(csr, ADF_GEN6_HIAEUNCERRLOG_CPP0, ae_uncorr_err);
	}
}

static void adf_handle_cpp_cmd_par_err(struct adf_accel_dev *accel_dev,
				       void __iomem *csr, u32 errsou)
{
	...
	cmd_par_err &= err_mask->cppagentcmdpar_mask;
	if (cmd_par_err) {
		dev_err(&GET_DEV(accel_dev),
				      ...
				      void __iomem *csr, u32 errsou)
{
	...
	rimem_parerr_sts &= ADF_GEN6_RIMEM_PARERR_STS_UNCERR_BITMASK |
			    ADF_GEN6_RIMEM_PARERR_STS_FATAL_BITMASK;
	if (rimem_parerr_sts & ADF_GEN6_RIMEM_PARERR_STS_UNCERR_BITMASK) {
		dev_err(&GET_DEV(accel_dev),
				     ...
				     void __iomem *csr, u32 errsou)
{
	...
	ti_ci_par_sts &= ADF_GEN6_TI_CI_PAR_STS_BITMASK;
	if (ti_ci_par_sts) {
		dev_err(&GET_DEV(accel_dev),
					  ...
					  void __iomem *csr, u32 errsou)
{
	...
	ti_pullfub_par_sts &= ADF_GEN6_TI_PULL0FUB_PAR_STS_BITMASK;
	if (ti_pullfub_par_sts) {
		dev_err(&GET_DEV(accel_dev),
			"TI Pull Parity Error: 0x%x\n", ti_pullfub_par_sts);
		ADF_CSR_WR(csr, ADF_GEN6_TI_PULL0FUB_PAR_STS,
			   ti_pullfub_par_sts);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
	}
}
					  ...
					  void __iomem *csr, u32 errsou)
{
	...
	ti_pushfub_par_sts &= ADF_GEN6_TI_PUSHFUB_PAR_STS_BITMASK;
	if (ti_pushfub_par_sts) {
		dev_err(&GET_DEV(accel_dev),
			"TI Push Parity Error: 0x%x\n", ti_pushfub_par_sts);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
		ADF_CSR_WR(csr, ADF_GEN6_TI_PUSHFUB_PAR_STS,
			   ti_pushfub_par_sts);
	}
}
				     ...
				     void __iomem *csr, u32 errsou)
{
	...
	ti_cd_par_sts &= ADF_GEN6_TI_CD_PAR_STS_BITMASK;
	if (ti_cd_par_sts) {
		dev_err(&GET_DEV(accel_dev),
					...
					void __iomem *csr, u32 errsou)
{
	...
	ti_trnsb_par_sts &= ADF_GEN6_TI_TRNSB_PAR_STS_BITMASK;
	if (ti_trnsb_par_sts) {
		dev_err(&GET_DEV(accel_dev),
...
static void adf_handle_sfi_cmd_parerr(struct adf_accel_dev *accel_dev,
				      void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"Command Parity error detected on SFI\n");

	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}

static void adf_gen6_process_errsou1(struct adf_accel_dev *accel_dev,
				     void __iomem *csr, u32 errsou)
{
	...
	adf_handle_ti_ci_par_sts(accel_dev, csr, errsou);
	adf_handle_ti_pullfub_par_sts(accel_dev, csr, errsou);
	adf_handle_ti_pushfub_par_sts(accel_dev, csr, errsou);
	adf_handle_ti_cd_par_sts(accel_dev, csr, errsou);
	adf_handle_ti_trnsb_par_sts(accel_dev, csr, errsou);
	adf_handle_iosfp_cmd_parerr(accel_dev, csr, errsou);
...
static void disable_ssm_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_INTMASKSSM, ADF_GEN6_INTMASKSSM_BITMASK);
}
...
static void disable_ti_ri_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_TI_CI_PAR_ERR_MASK,
		   ADF_GEN6_TI_CI_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_PULL0FUB_PAR_ERR_MASK,
		   ADF_GEN6_TI_PULL0FUB_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_PUSHFUB_PAR_ERR_MASK,
		   ADF_GEN6_TI_PUSHFUB_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_CD_PAR_ERR_MASK,
		   ADF_GEN6_TI_CD_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_TRNSB_PAR_ERR_MASK,
		   ADF_GEN6_TI_TRNSB_PAR_STS_BITMASK);

	/* Disable error handling in RI, TI CPP interface control registers */
	reg = ADF_CSR_RD(csr, ADF_GEN6_RICPPINTCTL);
	reg &= ~ADF_GEN6_RICPPINTCTL_BITMASK;
	ADF_CSR_WR(csr, ADF_GEN6_RICPPINTCTL, reg);

	reg = ADF_CSR_RD(csr, ADF_GEN6_TICPPINTCTL);
	reg &= ~ADF_GEN6_TICPPINTCTL_BITMASK;
	...
	reg &= ADF_GEN6_TIMSCCTL_RELAY_BITMASK;
...
static void disable_cpp_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_CPP_CFC_ERR_CTRL,
		   ADF_GEN6_CPP_CFC_ERR_CTRL_DIS_BITMASK);
}
...
static void enable_ti_ri_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_RI_MEM_PAR_ERR_EN0,
		   ADF_GEN6_RIMEM_PARERR_STS_FATAL_BITMASK |
		   ADF_GEN6_RIMEM_PARERR_STS_UNCERR_BITMASK);
				       ...
				       void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_CPP_CFC_ERR_CTRL,
		   ADF_GEN6_CPP_CFC_ERR_CTRL_BITMASK);
}
...
static void enable_errsou_reporting(void __iomem *csr)
{
	 ...
	 */
	ADF_CSR_WR(csr, ADF_GEN6_ERRMSK2,
		   ADF_GEN6_ERRSOU2_PM_INT_BIT);
...
static void adf_gen6_process_reset(struct adf_accel_dev *accel_dev,
				   void __iomem *csr,
				   bool *reset_required)
{
	u8 reset_type, device_state;

	u32 gensts = ADF_CSR_RD(csr, ADF_GEN6_GENSTS);

	device_state = FIELD_GET(ADF_GEN6_GENSTS_DEVICE_STATE_BITMASK, gensts);
	reset_type = FIELD_GET(ADF_GEN6_GENSTS_RESET_TYPE_BITMASK, gensts);
	if (device_state == ADF_GEN6_GENSTS_DEVHALT &&
	    reset_type == ADF_GEN6_GENSTS_PFLR)
		*reset_required = true;

	if (reset_type == ADF_GEN6_GENSTS_COLD_RESET)
		dev_err(&GET_DEV(accel_dev),
			"Fatal error cold reset required\n");
}
				   ...
				   void __iomem *csr, u32 errsou)
{
	...
	if (!(errsou & ADF_GEN6_ERRSOU3_RICPPINTSTS_BITMASK))
		return;

	ricppintsts = ADF_CSR_RD(csr, ADF_GEN6_RICPPINTSTS);
	ricppintsts &= ADF_GEN6_RICPPINTSTS_BITMASK;
	if (ricppintsts) {
		dev_err(&GET_DEV(accel_dev),
			"RI push pull error: 0x%x\n", ricppintsts);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
				   ...
				   void __iomem *csr, u32 errsou)
{
	...
	if (!(errsou & ADF_GEN6_ERRSOU3_TICPPINTSTS_BITMASK))
		return;

	ticppintsts = ADF_CSR_RD(csr, ADF_GEN6_TICPPINTSTS);
	ticppintsts &= ADF_GEN6_TICPPINTSTS_BITMASK;
	if (ticppintsts) {
		dev_err(&GET_DEV(accel_dev),
			"TI push pull error: 0x%x\n", ticppintsts);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
...
static void adf_handle_rlterror(struct adf_accel_dev *accel_dev,
				void __iomem *csr, u32 errsou)
{
	u32 rlterror;
	...
	rlterror &= ADF_GEN6_RLT_ERRLOG_BITMASK;
	if (rlterror) {
		dev_err(&GET_DEV(accel_dev),
			"Error in RLT Block: 0x%x\n", rlterror);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
		ADF_CSR_WR(csr, ADF_GEN6_RLT_ERRLOG, rlterror);
	}
}

static void adf_handle_vflr(struct adf_accel_dev *accel_dev,
			    void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"Uncorrectable error in VF\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
}

static void adf_handle_tc_vc_map_error(struct adf_accel_dev *accel_dev,
				       void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"Violation of PCIE TC VC mapping\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}

static void adf_handle_pcie_devhalt(struct adf_accel_dev *accel_dev,
				    void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"CPM is in DEVHALT due to an error in an incoming transaction\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}

static void adf_handle_pg_req_devhalt(struct adf_accel_dev *accel_dev,
				      void __iomem *csr, u32 errsou)
{
...
static void adf_handle_xlt_cpl_devhalt(struct adf_accel_dev *accel_dev,
				       void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"Error status for a Address translation request\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}

static void adf_handle_ti_int_err_devhalt(struct adf_accel_dev *accel_dev,
					  void __iomem *csr, u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"CPM is in DEVHALT due to an internal memory error in TI\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}

static void adf_gen6_process_errsou3(struct adf_accel_dev *accel_dev,
				     void __iomem *csr, u32 errsou)
{
...
static bool adf_gen6_handle_interrupt(struct adf_accel_dev *accel_dev,
				      bool *reset_required)
{
	void __iomem *csr = adf_get_pmisc_base(accel_dev);
	u32 errsou = ADF_CSR_RD(csr, ADF_GEN6_ERRSOU0);
	bool handled = false;

	*reset_required = false;

	if (errsou & ADF_GEN6_ERRSOU0_BIT) {
		adf_gen6_process_errsou0(accel_dev, csr);
		handled = true;
	}

	errsou = ADF_CSR_RD(csr, ADF_GEN6_ERRSOU1);
	if (errsou & ADF_GEN6_ERRSOU1_BITMASK) {
		adf_gen6_process_errsou1(accel_dev, csr, errsou);
		handled = true;
	}

	errsou = ADF_CSR_RD(csr, ADF_GEN6_ERRSOU2);
	if (errsou & ADF_GEN6_ERRSOU2_BITMASK) {
		adf_gen6_process_errsou2(accel_dev, csr, errsou);
		handled = true;
	}

	errsou = ADF_CSR_RD(csr, ADF_GEN6_ERRSOU3);
	if (errsou & ADF_GEN6_ERRSOU3_BITMASK) {
		adf_gen6_process_errsou3(accel_dev, csr, errsou);
		handled = true;
	}

	adf_gen6_process_reset(accel_dev, csr, reset_required);
