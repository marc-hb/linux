				  ...
				  u32 errsou)
{
	...
	if (ae_uncorr_err) {
		dev_err(&GET_DEV(accel_dev), "Uncorrectable error detected in AE: %#x\n",
			ae_uncorr_err);
		ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_UNCORR);
				      ...
				      u32 errsou)
{
	...
	rimem_parerr_sts &= ADF_GEN6_RIMEM_PARERR_STS_CERR_BITMASK |
			    ADF_GEN6_RIMEM_PARERR_STS_FATAL_BITMASK;
	if (rimem_parerr_sts & ADF_GEN6_RIMEM_PARERR_STS_CERR_BITMASK) {
...
static void disable_ti_ri_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_TI_CI_PAR_ERR_MASK, ADF_GEN6_TI_CI_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_PULL0FUB_PAR_ERR_MASK, ADF_GEN6_TI_PULL0FUB_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_PUSHFUB_PAR_ERR_MASK, ADF_GEN6_TI_PUSHFUB_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_CD_PAR_ERR_MASK, ADF_GEN6_TI_CD_PAR_STS_BITMASK);
	ADF_CSR_WR(csr, ADF_GEN6_TI_TRNSB_PAR_ERR_MASK, ADF_GEN6_TI_TRNSB_PAR_STS_BITMASK);
...
static void disable_cpp_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_CPP_CFC_ERR_CTRL, ADF_GEN6_CPP_CFC_ERR_CTRL_DIS_BITMASK);
}
...
static void enable_ti_ri_error_reporting(void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_RI_MEM_PAR_ERR_EN0,
		   ADF_GEN6_RIMEM_PARERR_STS_FATAL_BITMASK |
		   ADF_GEN6_RIMEM_PARERR_STS_CERR_BITMASK);
...
static void enable_cpp_error_reporting(struct adf_accel_dev *accel_dev, void __iomem *csr)
{
	...
	ADF_CSR_WR(csr, ADF_GEN6_HICPPAGENTCMDPARERRLOGENABLE, err_mask->cppagentcmdpar_mask);

	ADF_CSR_WR(csr, ADF_GEN6_CPP_CFC_ERR_CTRL, ADF_GEN6_CPP_CFC_ERR_CTRL_BITMASK);
}
				    ...
				    u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev),
		"QAT is in DEVHALT due to an error in an incoming transaction\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}
					  ...
					  u32 errsou)
{
	...
	dev_err(&GET_DEV(accel_dev), "QAT is in DEVHALT due to a TI internal memory error\n");
	ADF_RAS_ERR_CTR_INC(accel_dev->ras_errors, ADF_RAS_FATAL);
}
