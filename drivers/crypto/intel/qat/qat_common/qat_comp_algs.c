...
#define QAT_ZSTD_DRIVER		"qat_zstd"
#define QAT_ACOMP_PRIORITY	4001
#define ZSTD_ALGO		"zstd"
				      ...
				      void *resp)
{
	...
	if (res && (!(strncmp((crypto_tfm_alg_driver_name(crypto_acomp_tfm(tfm))),
			      QAT_ZSTD_DRIVER, QAT_ZSTD_DRIVER_LEN)))) {
				 ...
				 enum icp_qat_hw_compression_algo alg_name)
{
	...
	case ICP_QAT_HW_COMPRESSION_ALGO_ZSTD:
		if (HW_CAP_ZSTD(ctx->inst->accel_dev))
			return qat_comp_build_ctx(inst->accel_dev, ctx->comp_ctx, QAT_ZSTD);
	break;
	default:
...
#if IS_ENABLED (CONFIG_CRYPTO_DEV_QAT_6XXX)
...
#if IS_ENABLED (CONFIG_CRYPTO_DEV_QAT_6XXX)
{
	.base = {
		.cra_name = ZSTD_ALGO,
		.cra_driver_name = QAT_ZSTD_DRIVER,
		.cra_priority = QAT_ACOMP_PRIORITY,
		.cra_flags = CRYPTO_ALG_ASYNC | CRYPTO_ALG_ALLOCATES_MEMORY |
