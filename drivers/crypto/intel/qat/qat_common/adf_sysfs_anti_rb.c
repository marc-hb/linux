...
static ssize_t enforced_min_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	...
	err = adf_anti_rb_query(accel_dev, ENFORCED_MIN_SVN, &svn);
	if (err)
		return err;
			   ...
			   char *buf)
{
	...
	err = adf_anti_rb_query(accel_dev, ACTIVE_SVN, &svn);
...
static ssize_t permanent_min_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct adf_accel_dev *accel_dev;
	int err;
	...
	err = adf_anti_rb_query(accel_dev, PERMANENT_MIN_SVN, &svn);
	if (err)
		return err;
...
static struct attribute_group qat_svn_group = {
	.attrs = qat_svn_attrs,
	.name = "qat_svn",
};
