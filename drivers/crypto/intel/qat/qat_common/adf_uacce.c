 ...
 * ring queue mode, the bank is chosen based on different policies. For WQ, it
 * selects a bank that is not used by any other queue (ref_counter == 0). If no
 * such bank is available, EBUSY is returned. For UQ, the bank with the lowest
 * ref_counter is chosen. EBUSY may also be returned if all banks have reached
...
static int queue_get_bank(struct adf_accel_dev *accel_dev, u32 pasid)
{
	...
	for (i = 0; i < num_banks; i++) {
		...
		if (bank_data[i].ref_counter < min_ref_counter) {
			min_ref_counter = bank_data[i].ref_counter;
			bank_number = i;
		}
	}

	if (accel_dev->ring_queue_mode == ADF_RING_QUEUE_WQ)
		return -EBUSY;

	if (min_ref_counter == ADF_MAX_RP_UQ_REFERENCES)
		return -EBUSY;

	bank_data[bank_number].ref_counter++;
	return bank_number;
}
...
static int queue_put_bank(struct adf_uacce_bank_queue *bank_queue)
{
	...
	bank_data = accel_dev->uacce_data.bank_data;
	ref_counter = bank_data[bank_queue->bank_number].ref_counter;
	if (ref_counter == 0 || ref_counter > ADF_MAX_RP_UQ_REFERENCES)
		return -EFAULT;
...
EXPORT_SYMBOL_GPL(adf_uacce_is_enabled);
