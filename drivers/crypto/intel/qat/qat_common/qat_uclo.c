				...
				unsigned int fw_type)
{
	...
	if (handle->chip_info->fw_auth) {
		...
		if (handle->chip_info->dual_sign) {
			module_type = *(u32 *)image;
			hdr_len = *(u32 *)(image + ICP_QAT_DUALSIGN_HDR_LEN_OFFSET);
			hdr_ver = *(u32 *)(image + ICP_QAT_DUALSIGN_HDR_VER_OFFSET);
					     ...
					     struct icp_qat_fw_auth_desc **desc)
{
	...
	virt_addr = (char *)dram_desc->dram_base_addr_v + sizeof(struct icp_qat_auth_chunk);
	bus_addr  = dram_desc->dram_bus_addr + sizeof(struct icp_qat_auth_chunk);
